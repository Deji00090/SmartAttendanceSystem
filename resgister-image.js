  ...
  //     formData.append('Photo', capturedBlob, 'face-capture.jpg');
...
// CAMERA STATE
// ============================
let capturedBlob  = null;   // the JPEG blob from canvas snapshot
...
async function startCamera() {
  ...
  try {
    ...
    video.onloadedmetadata = () => {
      video.play();
      camStatus.textContent = 'Position your face inside the oval';
...
function switchCamera() {
  ...
  // mirror only front camera so it feels like a natural selfie
...
function capturePhoto() {
  ...
  // un-mirror front-cam so the saved image is the correct orientation
  if (facingMode === 'user') {
    ctx.translate(w, 0);
    ctx.scale(-1, 1);
  }
  ...
  canvas.toBlob(blob => {
    ...
    document.getElementById('previewName').textContent = 'Photo captured ✓';
...
async function submitPhoto() {
  ...
  try {
    const formData = new FormData();
    // same field name your backend expects
    formData.append('Photo', capturedBlob, 'face-capture.jpg');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // start camera once page is ready
  startCamera();
});